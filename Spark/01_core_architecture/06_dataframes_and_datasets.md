...
| :--- | :--- | :--- |
| **Predicate Pushdown** | يُنزل الفلاتر لأقرب مصدر بيانات | يُرسل `WHERE amount > 1000` لقارئ Parquet ليستفيد من إحصاءات Row Groups/Pages ويقلل I/O |
...
- **GC أقل:** يقل object allocation والضغط على الـ Heap، لكن التنفيذ لا يصبح خالياً تماماً من GC.
...
**النتيجة:** يقل عدد استدعاءات الـ virtual functions وإنشاء الكائنات، ويستفيد JIT من loop أبسط. التحسن يعتمد على الاستعلام والبيانات ولا يوجد رقم ثابت لكل حالة.
...
| :--- | :--- | :--- | :--- |
| **السرعة** | غالباً الأسرع | قد يكون أبطأ عند استخدام typed lambdas بسبب Encoders | قريب من DataFrame JVM execution عند استخدام built-in expressions؛ أبطأ عند Python UDFs |
...
### الخطأ 2: قراءة أعمدة أكثر من المطلوب قبل عمليات مُكلفة

```python
# ❌ يوسع الخطة إذا استخدمت لاحقاً أعمدة كثيرة أو كتبتها كلها
df.select("*").groupBy("store_id").count()
# Spark SQL يستطيع أحياناً حذف الأعمدة غير المستخدمة، لكن لا تعتمد على ذلك في pipelines مع UDFs أو writes.
...
**الإجابة النموذجية:**
`UnsafeRow` هو تمثيل ثنائي مُضغوط لصف البيانات في مشروع Tungsten. بدلاً من تمثيل كل صف كسلسلة Java Objects، يستخدم Spark layout ثنائياً يقلل object overhead ويحسن CPU cache locality. قد تكون البيانات on-heap أو off-heap حسب الإعدادات، لذلك العبارة الدقيقة أنه يقلل ضغط GC ولا يلغيه دائماً.
